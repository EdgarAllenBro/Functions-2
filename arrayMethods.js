...
// CODE HERE
const totalPopulation = populations.reduce((pop1,pop2)=>{
  return pop1 + pop2
})
...
// CODE HERE
const totals = []
const addTax = orders.forEach((order)=>{
  let total = order.price * order.tax + order.price
  totals.push(total)
})
console.log(totals)
...
// CODE HERE
const itemPrice = []
const bobsTotal = purchases.filter((item)=>{
  if(item.owner === `Bob`)
  itemPrice.push(item.price)
  
})

const total = itemPrice.reduce((price1,price2)=>{
